...
import responseBodyTemplate from "../utils/responseBodyTemplate.js";
...
const fetchMovieByID = async (req, res) => {
    try {
        ...
        console.log('Error: ', error);
        return res.status(500).json(responseBodyTemplate.errorBody(error, error.name || `Something went wrong while fetching movie - ${req.params.id}`));
    }
}
...
const fetchAllMovies = async (req, res) => {
    try {
        const response = await MovieServices.fetchMovie(req.query.name ? { 'name': req.query.name } : {});
        ...
        console.log('Error: ', error);
        return res.status(500).json(responseBodyTemplate.errorBody(error, error.name || "Something went wrong while fetching the movie details."));
    }
}
